...
import { Avatar, AvatarFallback, AvatarImage } from '@tazeai/ui/components/avatar';
...
export default function UserManagementList() {
  ...
  const columns: Column<User>[] = [
    ...
    },
    {
      id: 'status',
      header: '状态',
      cell: (user) => <Badge className={getStatusBadgeColor(user.status)}>{user.status}</Badge>,
    },
    {
      id: 'lastLogin',
      header: '最后登录',
  ...
  const bulkActions = (
    <div
      className={cn(
        ...
        selectedRowIds.length > 0 ? 'bg-muted/50 border rounded-lg p-2 shadow-sm' : 'h-0 overflow-hidden p-0',
      )}
    >
