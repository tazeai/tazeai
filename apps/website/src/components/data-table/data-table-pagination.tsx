...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@tazeai/ui/components/select';
...
export function DataTablePagination({
  ...
  const generatePaginationButtons = () => {
    ...
    if (startPage > 2) {
      buttons.push(
        <span key="ellipsis-start" className="flex items-center justify-center w-8 h-8 text-muted-foreground">
          ···
        </span>,
      );
    }
    ...
    if (endPage < totalPages - 1) {
      buttons.push(
        <span key="ellipsis-end" className="flex items-center justify-center w-8 h-8 text-muted-foreground">
          ···
        </span>,
      );
    }
    ...
    <div className="mt-4 border-t pt-4">
      <div className="flex flex-col lg:flex-row items-center justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6 w-full lg:w-auto order-2 lg:order-1">
          ...
          <div className="text-sm text-muted-foreground whitespace-nowrap">
            显示 <span className="font-medium">{startItem}</span> - <span className="font-medium">{endItem}</span> 共{' '}
          ...
          <div className="flex items-center">
            <div className="flex items-center">
              <Button
                variant="outline"
                size="icon"
                className={cn('h-8 w-8 rounded-r-none border-r-0', currentPage === 1 && 'opacity-50')}
                onClick={() => onPageChange(1)}
                disabled={currentPage === 1}
                aria-label="首页"
              >
              ...
              <Button
                variant="outline"
                size="icon"
                className={cn('h-8 w-8 rounded-l-none rounded-r-none', currentPage === 1 && 'opacity-50')}
            ...
            <div className="flex items-center">
              <Button
                variant="outline"
                size="icon"
                className={cn('h-8 w-8 rounded-l-none rounded-r-none', currentPage === totalPages && 'opacity-50')}
              ...
              <Button
                variant="outline"
                size="icon"
                className={cn('h-8 w-8 rounded-l-none', currentPage === totalPages && 'opacity-50')}
                onClick={() => onPageChange(totalPages)}
                disabled={currentPage === totalPages || totalPages === 0}
                aria-label="末页"
              >
      ...
      <div className="md:hidden flex justify-center mt-2">
        <span className="text-sm text-muted-foreground">
          第 <span className="font-medium">{currentPage}</span> 页，共 <span className="font-medium">{totalPages}</span>{' '}
          页
        </span>
      </div>
    </div>
