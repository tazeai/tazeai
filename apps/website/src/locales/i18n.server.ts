...
import { initializeServerI18n, parseAcceptLanguageHeader } from '@tazeai/i18n/server';
...
function getLanguageOrFallback(language: string | undefined) {
  ...
  if (!languages.includes(language ?? '')) {
    console.warn(`Language "${language}" is not supported. Falling back to "${languages[0]}"`);
